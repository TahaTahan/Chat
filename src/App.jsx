...
// ── Memory Modal ──────────────────────────────────────────────────────────────
function MemoryModal({ memories, onSave, onUpdate, onDelete, onClose }) {
  const [tab, setTab] = useState(memories.length > 0 ? "view" : "add");
  const [newText, setNewText] = useState("");
  const [editingIdx, setEditingIdx] = useState(null);
  const [editText, setEditText] = useState("");

  const startEdit = (idx) => { setEditingIdx(idx); setEditText(memories[idx]); };
  const cancelEdit = () => { setEditingIdx(null); setEditText(""); };
  const saveEdit = () => { if (editText.trim()) { onUpdate(editingIdx, editText.trim()); setEditingIdx(null); setEditText(""); } };

  const tabBtn = (id, label) => (
    <button onClick={() => setTab(id)} style={{
      flex: 1, padding: "8px 0",
      background: tab === id ? "linear-gradient(135deg, #8b6914, #c49a28)" : "transparent",
      border: "none", borderBottom: tab === id ? "none" : "1px solid #3a2e22",
      color: tab === id ? "#1a1208" : "#8a7a6a",
      cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 13,
      fontWeight: tab === id ? "bold" : "normal",
      borderRadius: tab === id ? "8px 8px 0 0" : "0", transition: "all 0.2s"
    }}>{label}</button>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: 16, width: 400, maxWidth: "92vw", maxHeight: "85vh",
        ...
        <div style={{ padding: "20px 24px 0", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <h3 style={{ margin: 0, color: "#e8d5b0", fontFamily: "'Playfair Display', serif", fontSize: 20 }}>
              Memories {memories.length > 0 && <span style={{ color: "#8b6914", fontSize: 14, fontWeight: "normal" }}>({memories.length})</span>}
            </h3>
            <button onClick={onClose} style={{ background: "transparent", border: "1px solid #3a2e22", color: "#8a7a6a", borderRadius: 7, padding: "4px 10px", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 13 }}>✕ Close</button>
          </div>
          <div style={{ display: "flex", gap: 2 }}>
            {memories.length > 0 && tabBtn("view", `View & Edit (${memories.length})`)}
            {tabBtn("add", "+ Add New")}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px 24px" }}>
          ...
          {tab === "view" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {memories.length === 0 ? (
                <p style={{ color: "#5a4a3a", fontFamily: "Georgia, serif", fontSize: 14, textAlign: "center", margin: "20px 0" }}>No memories saved yet.</p>
              ) : memories.map((mem, idx) => (
                <div key={idx} style={{ background: "#0f0d0a", border: "1px solid #3a2e22", borderRadius: 10, padding: "12px 14px" }}>
                  {editingIdx === idx ? (
                    <>
                      <textarea
                        autoFocus value={editText} onChange={e => setEditText(e.target.value)}
                        style={{
                          width: "100%", minHeight: 80, background: "#1a1610",
                          border: "1px solid #c49a28", borderRadius: 8, color: "#d4c4a0",
                          fontFamily: "Georgia, serif", fontSize: 13, padding: 10,
                          resize: "vertical", outline: "none", boxSizing: "border-box", lineHeight: 1.6
                        }}
                      />
                      <div style={{ display: "flex", gap: 8, marginTop: 8, justifyContent: "flex-end" }}>
                        <button onClick={cancelEdit} style={{ background: "transparent", border: "1px solid #3a2e22", color: "#8a7a6a", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 12 }}>Cancel</button>
                        <button onClick={saveEdit} disabled={!editText.trim()} style={{ background: editText.trim() ? "linear-gradient(135deg, #8b6914, #c49a28)" : "#2a2018", border: "none", color: editText.trim() ? "#1a1208" : "#5a4a3a", borderRadius: 6, padding: "5px 14px", cursor: editText.trim() ? "pointer" : "default", fontFamily: "Georgia, serif", fontSize: 12, fontWeight: "bold" }}>Save</button>
                      </div>
                    </>
                  ) : (
                    <>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
                        <span style={{ color: "#8b6914", fontSize: 11, fontFamily: "Georgia, serif", flexShrink: 0 }}>#{idx + 1}</span>
                        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                          <button onClick={() => startEdit(idx)} style={{ background: "transparent", border: "1px solid #3a2e22", color: "#c49a28", borderRadius: 6, padding: "3px 9px", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 11 }}>✎ Edit</button>
                          <button onClick={() => onDelete(idx)} style={{ background: "transparent", border: "1px solid #3a1a1a", color: "#8a4a4a", borderRadius: 6, padding: "3px 9px", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 11 }}>✕</button>
                        </div>
                      </div>
                      <p style={{ margin: 0, color: "#d4c4a0", fontFamily: "Georgia, serif", fontSize: 13, lineHeight: 1.65 }}>{mem}</p>
                    </>
                  )}
                </div>
              ))}
          ...
          {tab === "add" && (
            <>
              <p style={{ margin: "0 0 12px", color: "#8a7a6a", fontSize: 13, fontFamily: "Georgia, serif" }}>
                This text will be permanently appended to the character history.
              </p>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 14, justifyContent: "flex-end" }}>
                <button
                  onClick={() => { if (newText.trim()) { onSave(newText.trim()); setNewText(""); setTab("view"); } }}
                  disabled={!newText.trim()}
                  style={{
                    background: newText.trim() ? "linear-gradient(135deg, #8b6914, #c49a28)" : "#2a2018",
                    border: "none", color: newText.trim() ? "#1a1208" : "#5a4a3a",
                    borderRadius: 8, padding: "9px 22px", cursor: newText.trim() ? "pointer" : "default",
...
function ChatScreen({ config, onReset }) {
  ...
  const [histEditIdx, setHistEditIdx] = useState(null);   // index of msg being edited
  const [histEditText, setHistEditText] = useState("");
  ...
  // ── Historical message edit/delete ──────────────────────────────────────────
  const startHistEdit = (idx) => {
    setHistEditIdx(idx);
    setHistEditText(messages[idx].content);
  };
  const cancelHistEdit = () => { setHistEditIdx(null); setHistEditText(""); };

  const saveHistEdit = async (idx) => {
    const msg = messages[idx];
    const newContent = histEditText.trim();
    if (!newContent) return;

    if (msg.role === "assistant") {
      // Just update in place — no API call needed
      setMessages(prev => prev.map((m, i) => i === idx ? { ...m, content: newContent } : m));
      setHistEditIdx(null);
      setHistEditText("");
      return;
    }

    // User message — truncate history from this point and regenerate
    const truncated = messages.slice(0, idx); // everything before this message
    setMessages(truncated);
    setHistEditIdx(null);
    setHistEditText("");
    cancelHistEdit();

    // Clear any pending exchange
    setPendingUser(null);
    setVariants([]);
    setActiveVariant(0);
    ...
    try {
      const reply = await callApi(truncated, newContent);
      setPendingUser(newContent);
      setVariants([reply]);
      setActiveVariant(0);
    } catch (e) {
      setPendingUser(newContent);
  ...
  const deleteHistMsg = (idx) => {
    // Delete the message and its pair (user+assistant always come in pairs)
    setMessages(prev => {
      const arr = [...prev];
      const msg = arr[idx];
      if (msg.role === "user") {
        // Delete this user msg + the next assistant msg if it exists
        const end = idx + 1 < arr.length && arr[idx + 1].role === "assistant" ? idx + 2 : idx + 1;
        arr.splice(idx, end - idx);
      } else {
        // Delete just this assistant msg
        arr.splice(idx, 1);
      }
      return arr;
    });
    // Clear any pending exchange to avoid confusion
    setPendingUser(null);
    setVariants([]);
    setActiveVariant(0);
    setHistEditIdx(null);
  ...
  const updateMemory = (idx, text) => {
    setMemories(prev => prev.map((m, i) => i === idx ? text : m));
  };
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", position: "relative", ...bgStyle }}>
      ...
      {/* ── Header ── */}
      <div style={{ position: "sticky", top: 0, zIndex: 20, background: "rgba(10,8,5,0.88)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(90,74,58,0.4)" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 16px", display: "flex", flexDirection: "column", gap: 14, position: "relative", zIndex: 5 }}>
              ...
              {isEditing ? (
                <div style={{ maxWidth: "78%", width: "78%" }}>
                  <textarea
                    autoFocus
                    value={histEditText}
                    onChange={e => setHistEditText(e.target.value)}
                    style={{
                      width: "100%", minHeight: 60,
                      background: isUser ? "rgba(139,105,20,0.3)" : "rgba(15,12,8,0.9)",
                      border: `1px solid ${isUser ? "rgba(196,154,40,0.6)" : "rgba(90,74,58,0.6)"}`,
                      borderRadius: isUser ? "18px 18px 4px 18px" : "18px 18px 18px 4px",
                      padding: "11px 15px", color: isUser ? "#e8d0a0" : "#d4c4a0",
                      fontFamily: "Georgia, serif", fontSize: 15, lineHeight: 1.65,
                      outline: "none", resize: "vertical", boxSizing: "border-box"
                    }}
                  />
                  <div style={{ display: "flex", gap: 6, justifyContent: isUser ? "flex-end" : "flex-start", marginTop: 5 }}>
                    <button onClick={cancelHistEdit} style={btnStyle("red")}>Cancel</button>
                    {isUser
                      ? <button onClick={() => saveHistEdit(i)} disabled={!histEditText.trim() || histEditLoading} style={{ ...btnStyle("gold"), opacity: !histEditText.trim() || histEditLoading ? 0.4 : 1 }}>↑ Resend</button>
                      : <button onClick={() => saveHistEdit(i)} disabled={!histEditText.trim()} style={{ ...btnStyle("gold"), opacity: !histEditText.trim() ? 0.4 : 1 }}>✓ Save</button>
                    }
                  </div>
                </div>
              ) : (
                <>
                  <div style={{
                    maxWidth: "78%",
                    background: isUser ? "linear-gradient(135deg, rgba(139,105,20,0.85), rgba(196,154,40,0.75))" : "rgba(15,12,8,0.82)",
                    backdropFilter: "blur(12px)",
                    border: isUser ? "1px solid rgba(196,154,40,0.4)" : "1px solid rgba(90,74,58,0.35)",
                    borderRadius: isUser ? "18px 18px 4px 18px" : "18px 18px 18px 4px",
                    padding: "11px 15px",
                    color: isUser ? "#1a1208" : "#d4c4a0",
                    fontFamily: "Georgia, serif", fontSize: 15, lineHeight: 1.65,
                    boxShadow: "0 4px 20px rgba(0,0,0,0.4)"
                  }}>
                    {msg.content}
                  </div>
                  {/* Edit/Delete buttons — shown below each message */}
                  <div style={{ display: "flex", gap: 5 }}>
                    <button
                      onClick={() => startHistEdit(i)}
                      disabled={histEditIdx !== null || loading}
                      style={{ ...btnStyle("gold"), opacity: histEditIdx !== null || loading ? 0.4 : 1 }}
                    >✎ Edit</button>
                    <button
                      onClick={() => deleteHistMsg(i)}
                      disabled={histEditIdx !== null || loading}
                      style={{ ...btnStyle("red"), opacity: histEditIdx !== null || loading ? 0.4 : 1 }}
                    >✕ Del</button>
                  </div>
                </>
              )}
        ...
        {/* Hist-edit loading indicator */}
        {histEditLoading && (
          <div style={{ display: "flex", justifyContent: "flex-start" }}>
            <div style={{ background: "rgba(15,12,8,0.82)", backdropFilter: "blur(12px)", border: "1px solid rgba(90,74,58,0.35)", borderRadius: "18px 18px 18px 4px", padding: "11px 15px", color: "#8a7a6a", fontFamily: "Georgia, serif", fontSize: 13 }}>
              Rewriting from that point…
            </div>
          </div>
        )}
      ...
      {showMemoryModal && <MemoryModal memories={memories} onSave={addMemory} onUpdate={updateMemory} onDelete={deleteMemory} onClose={() => setShowMemoryModal(false)} />}
