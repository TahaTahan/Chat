...
// ── Reply Length Picker (inline row) ─────────────────────────────────────────
      ...
      {REPLY_LENGTHS.map(opt => {
        ...
        return (
          <button
            ...
            {compact ? opt.icon : `${opt.icon} ${opt.label}`}
          </button>
        );
      })}
    </div>
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", position: "relative", ...bgStyle }}>
      ...
      <div style={{
        ...
        {/* Expandable reply length bar */}
        {showLengthBar && (
          <div style={{
            borderTop: "1px solid rgba(90,74,58,0.3)",
            padding: "10px 14px",
            ...
            <ReplyLengthPicker value={replyLength} onChange={(v) => { setReplyLength(v); }} />
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        <textarea
          ...
          placeholder={`Message ${config.charName}...`}
          rows={1}
          style={{
